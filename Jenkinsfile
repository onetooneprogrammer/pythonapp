...
pipeline {
    ...
    stages {
        stage('Checkout') { // Stage to checkout the code
            steps {
                git url: 'https://github.com/onetooneprogrammer/pythonapp.git', branch: 'master' // For local git repo. For remote: 'https://github.com/your_username/my_python_app.git'
        ...
        stage('Run Tests') { // Stage to run our Python unit tests
            steps {
                sh '. venv/bin/activate && python3 -m unittest discover -s . -p "test_*.py"'                // This command discovers and runs tests in files named test_*.py
